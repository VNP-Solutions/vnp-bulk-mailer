...
/**
 * Permanently delete a user. Only allowed once a user has been revoked —
 * an active/pending user must be revoked first. You can't delete yourself.
 */
async function deleteUser({ userId, actorId }) {
    if (String(userId) === String(actorId)) {
        const err = new Error("You can't delete your own account");
        err.statusCode = 400;
        throw err;
    }
    const user = await User.findById(userId);
    ...
    if (user.status !== 'revoked') {
        const err = new Error('Only revoked users can be deleted. Revoke them first.');
        err.statusCode = 400;
        throw err;
    }
