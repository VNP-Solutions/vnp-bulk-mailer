...
function getTransporter() {
    ...
    // Dev/test hook: EMAIL_TRANSPORT=json serializes messages instead of
    // delivering them, so the send pipeline can be exercised without SMTP.
...
function buildInviteHtml({ firstName, inviterName, inviteUrl, expiresInDays }) {
    ...
    const fromLine = inviterName
        ? `${inviterName} has invited you to the VNP Email Utility.`
        : `You've been invited to the VNP Email Utility.`;
    return `
<!DOCTYPE html>
<html>
<head>
...
async function sendInviteEmail({ to, firstName, inviterName, inviteUrl, expiresInDays }) {
    ...
    await mailer.sendMail({
        ...
        to,
        subject: inviterName
            ? `${inviterName} invited you to VNP Email Utility`
            : `You've been invited to VNP Email Utility`,
        text: `You've been invited to the VNP Email Utility. Accept your invitation here: ${inviteUrl} (expires in ${expiresInDays} days)`,
