(() => {
    ...
    function renderRows(users) {
        ...
        users.forEach((user, i) => {
            ...
            // Action: revoke active/pending users; delete once revoked.
            let actionsHtml;
            if (isYou) {
                actionsHtml = '<span class="action-meta">That\'s you</span>';
            } else if (status === 'revoked') {
                actionsHtml = `<button class="action-link" data-action="delete" data-id="${user._id}">Delete</button>`;
            } else {
    ...
    tbody.addEventListener('click', (e) => {
        ...
        if (!user) return;
        deleteTargetId = id;
        deleteName.textContent = fullName(user);
    ...
    deleteSubmit.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`/api/users/${deleteTargetId}/permanent`, {
                method: 'DELETE',
                headers: authHeaders(),
            });
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                deleteError.textContent = data.error || 'Could not delete user.';
    ...
    document.addEventListener('keydown', (e) => {
        ...
        [addModal, inviteModal, revokeModal, deleteModal].forEach((m) => {
